...
from super.service import fetch_group_name
...
# AFTER
def validate_super_user_role(user):
    if user is None:
...
async def validate_group_access(user_id, group_id, db):
    from super.service import fetch_group_list
    group = await fetch_group_list(user_id, db)
    isGroup = False
    for g in group:
        if g.id == group_id:
            isGroup = True
...
def validate_student_group_access(group_list, std_team_id):
    isGroup = False
    for u in group_list:
        if u.id == std_team_id:
            isGroup = True
